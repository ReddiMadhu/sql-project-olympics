...
class InsuranceCorrelationAnalyzer:
    ...
    DEFAULT_CATEGORICAL = [
        ...
        "ZIP",                  # ZIP code - categorical (location identifier)
        "ACCOUNTING_MONTH",     # Month - categorical (temporal category)
        "FULLPOLICY_NB",        # Policy number - categorical (identifier)
    ]
    ...
    DEFAULT_NUMERICAL = [
        ...
        "GROSSLOSSPAIO_AM",     # Gross loss paid amount - numerical (financial)
    ]
    ...
    def _conditional_distributions(self):
        """Analyze conditional distributions for synthetic data generation."""
        print("Computing conditional distributions for synthetic data...")

        categorical_cols, numeric_cols, _ = self._get_available_columns()

        conditional_stats = {}

        # For each important numerical column, get stats conditioned on categorical columns
        # Focus on key columns for insurance synthetic data
        key_numeric = ['GROSSLOSSPAIO_AM', 'EARNEDPREMIUM_AM', 'DIRECTWRITTENPREMIUNM_AM']
        key_categorical = ['POLICYRATEDSTATE_TP', 'ZIP', 'CONSTRUCTION_TP', 'ROOF_TP']

        for num_col in key_numeric:
            if num_col not in numeric_cols:
                continue

            conditional_stats[num_col] = {}
            ...
            for cat_col in key_categorical:
                ...
                numeric_data = pd.to_numeric(self.data[num_col], errors='coerce')
                grouped = pd.DataFrame({
                    'cat': self.data[cat_col],
                    'val': numeric_data
                }).dropna().groupby('cat')['val']

                stats_by_category = {}
                for category, group in grouped:
                    if len(group) >= 5:  # At least 5 samples
                        stats_by_category[str(category)] = {
                ...
                if stats_by_category:
                    conditional_stats[num_col][cat_col] = stats_by_category

        self.correlations['conditional_distributions'] = conditional_stats
        print(f"  Computed conditional distributions for synthetic data generation")
    ...
    def print_summary(self):
        ...
        print("\n" + "=" * 80)
        print("CORRELATION ANALYSIS SUMMARY - FOR SYNTHETIC DATA GENERATION")
        print("=" * 80)
        ...
        if 'strong_numerical_correlations' in self.correlations:
            ...
            print("  → Use these to maintain realistic relationships in synthetic data")
        ...
        if 'categorical_associations' in self.correlations:
            ...
            print("  → Preserve these category combinations in synthetic data")
        ...
        if 'categorical_numerical_relationships' in self.correlations:
            ...
            print("  → Generate numerical values conditioned on categorical values")
            for rel in cat_num[:5]:  # Top 5
                print(f"  {rel['categorical']} → {rel['numerical']}: η² = {rel['eta_squared']:.3f}")

        # Conditional distributions
        if 'conditional_distributions' in self.correlations:
            cond_dist = self.correlations['conditional_distributions']
            print(f"\nConditional Distributions: {len(cond_dist)} numerical columns analyzed")
            print("  → Use these statistics to generate realistic values per category")
            for num_col, cat_dict in list(cond_dist.items())[:3]:
                print(f"  {num_col} varies by: {', '.join(cat_dict.keys())}")

        print("\n" + "=" * 80)
        print("RECOMMENDATIONS FOR SYNTHETIC DATA:")
        print("  1. Use conditional distributions to sample numerical values")
        print("  2. Maintain categorical associations (e.g., state-ZIP combinations)")
        print("  3. Preserve correlations between numerical columns")
        print("  4. Generate data category-by-category for realistic patterns")
        print("=" * 80 + "\n")
