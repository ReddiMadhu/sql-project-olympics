...
SAFE_REWRITES = [
    ...
    ("a + b", "SUM(a, b)"),                                       # literal arithmetic, no row context
]
...
```python
def determine_recommendation(member_id, golden_id, pairwise_results, subsumption_map):
    if member_id == golden_id:
        return "keep"

    subsumption = subsumption_map.get((member_id, golden_id))

    if subsumption == f"{member_id}⊂{golden_id}":
        # Simple case: golden already contains everything. Safe to decommission outright.
        return "decommission"

    if subsumption == f"{golden_id}⊂{member_id}":
        # Golden is structurally a SUBSET of this member — usage/freshness picked
        # golden anyway. Golden must absorb member's unique content before member
        # can be decommissioned. Not a flat "keep" or "decommission."
        return "merge_content_into_golden"

    if subsumption is None:
        # Neither subsumes the other (partial overlap only) — standard merge review.
        pair_composite = get_composite(member_id, golden_id, pairwise_results)
        return "merge" if pair_composite > 0.80 else "review"

    return "review"  # fallback for any unhandled subsumption state
```
...
3. Group sources by schema fingerprint similarity:
   - **PostgreSQL (prod)**: Use `pg_trgm` `similarity()` function on concatenated normalized column names. Two sources are in the same block if `similarity(fingerprint_a, fingerprint_b) > 0.6` or exact hash match.
...
def bands_rows_for_threshold(target_j, num_perm=256):
    ...
    inflection point is closest to target_j, biased toward more bands
    (higher recall, more false positives — acceptable since Stage 2 is exact)."""
    best = None
    for r in range(1, num_perm + 1):
        ...
        b = num_perm // r
        inflection = (1 / b) ** (1 / r)
        if inflection <= target_j:
            score = target_j - inflection
...
```python
def compare_dax_measures(measure_a, measure_b):
        ...
        for cluster_id, member_ids in cluster_assignments.items():
            ...
            for member_id in member_ids:
                recommendation = determine_recommendation(
                    member_id, golden, pairwise_results, subsumption_map
                )
...
```python
def incremental_update(changed_report_ids: list, all_reports: list, db: Database):
    """
    Scoped recompute: a new/edited report doesn't trigger a full N² re-run.
    """
    # 1. Recompute fingerprints only for changed reports
    for report_id in changed_report_ids:
    ...
    # 2. Re-run blocking + LSH candidate generation ONLY for changed reports
    #    against the full existing corpus — not full corpus against itself
    new_candidate_pairs = set()
    for report_id in changed_report_ids:
        block = get_logical_block(report_id)
        block_reports = get_reports_in_block(block)  # existing + changed
    ...
    for pair in new_candidate_pairs:
        ...
        db.upsert_pairwise_result(result)  # idempotent upsert, not insert

    # 4. Stale pairwise rows: any pair involving a changed report where the OTHER
    #    report hasn't changed still needs re-scoring, since the report itself changed
    stale_pairs = db.get_pairwise_rows_touching(changed_report_ids)
    for pair in stale_pairs:
        if pair not in new_candidate_pairs:
            # was a candidate before, may no longer be — re-verify rather than drop silently
            revalidate_or_retire(pair, db)

    # 5. Invalidate and recompute cluster membership ONLY for clusters touching
    #    the changed reports (not global re-clustering)
